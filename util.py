...
def normalize_data(inp):
    ...
    def normalize_single_image(image):
        """
        Normalize a single image by subtracting the mean and dividing by the standard deviation.
        ...
        """
        mean = np.sum(image)/(len(image))
        std = np.std(image)
        norm_image = (image - mean)/std

        return norm_image
    
    norm_inps = np.array([normalize_single_image(image) for image in inp])
...
def one_hot_encoding(labels, num_classes=10):
    ...
    """
    # Create an array of zeros N X num_classes
    one_hot = np.zeros((labels.size, num_classes), dtype=int)
    # Set each label to 1 for the corresponding column
...
def createTrainValSplit(x_train,y_train):
    ...
    Also shuffles the data before splitting.
    args:
    ...
    """
    # Get the indices of the training data and shuffle them
    ...
    train_indices, val_indices = indices[:num_train_samples], indices[num_train_samples:]
