...
plotting_probs = ['test_softmax', 'test_momentum', 'test_regularization', 'test_activation']
...
def main(args):
    ...
    if args.experiment in plotting_probs:
        # train the model
        model = train(model, x_train, y_train, x_valid, y_valid, config, plot=True)
    else:
        # train the model
